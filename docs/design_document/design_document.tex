...
Neo4j database is created by a python script that takes data directly from the document database: useful data are first withdrawn from mongoDB, then they are post-processed in order to filter only the necessaries attributes, and then they are merged into neo4j. This can be done thanks to the \textit{pymongo} and \textit{py2neo} modules.

The script manage the consistency of 3 out of the 4 elements present in our graph database:
\begin{itemize}
	\item vertex \textbf{Movie};
	\item vertex \textbf{User};
	\item edge \textbf{Rating}.
\end{itemize}
Edge \textbf{Follows} is not managed in this script because it only exists in the graph database.
...
to a subset of 1000 movies (instead of the full \~50k movies).
